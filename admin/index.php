...
$totalImp = 0; $totalViews = 0; $totalHits = 0; $totalSpent = 0; $totalBalance = 0;
foreach ($campaigns as $c) {
    $totalImp   += $c['impressions'] ?? 0;
    $totalViews += $c['good_hits']   ?? 0;
    $totalHits  += $c['clicks']      ?? 0;   // clicks field = hits in UI
...
<div class="stats-grid">
    ...
    <div class="stat-card">
        <div class="stat-icon" style="background:rgba(255,149,0,.1);color:var(--orange)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div>
    ...
    <div class="stat-card">
        <div class="stat-icon" style="background:rgba(0,208,132,.1);color:var(--green)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></div>
    ...
    <div class="stat-card">
        <div class="stat-icon" style="background:rgba(0,208,132,.1);color:var(--green)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg></div>
        <div class="stat-label">Total Views</div>
        <div class="stat-value" data-live="total-views"><?= fmtNum($totalViews) ?></div>
    </div>
    <div class="stat-card">
        ...
        <div class="stat-value" data-live="total-hits"><?= fmtNum($totalHits) ?></div>
    </div>
    <div class="stat-card">
...
<div class="card">
    ...
    <div style="display:grid;gap:10px">
        ...
        <a href="/admin/campaigns.php?filter=all" class="btn btn-secondary" style="justify-content:flex-start">
...
<div class="card">
    ...
    <?php foreach ($activity as $a):
        ...
        $label = $a['status'] === 'review' ? 'Under Review' : $a['status'];
    ?>
    <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border)">
        ...
        <span class="badge <?= $sCls ?>"><?= $label ?></span>
    </div>
    <?php endforeach; ?>
    <?php endif; ?>
</div>
